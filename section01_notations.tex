...
\}.
\]
In this paper, we use $\operatorname{relint}$ instead of $\operatorname{int}$ to properly handle the singleton set $\{0\}$:
we have $\operatorname{relint}(\{0\}) = \{0\}$ while $\operatorname{int}(\{0\}) = \emptyset$.
For all proper cones considered in this paper ($\mathbb{R}^n_+$, $\mathbb{Q}^n$, $\mathbb{S}^n_+$), the two notions coincide.
